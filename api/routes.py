...
# Post booking from pro
@api.route("/new_pro_booking", methods=['POST'])
def handle_bookings():
    data = request.json
    ...
    if patient and booking:
        ...
        is_patient = Patients.query.filter_by(email=patient_email).first()
        if is_patient:
            try:
                if patient['email'] != is_patient.email or \
                patient['name'] != is_patient.name or \
                patient['phone'] != is_patient.phone:
                    update_patient(patient, is_patient)
            except Exception as e:
                return jsonify({"error": f"Error while updating patient: {str(e)}"}), 500
            try:
                new_booking = create_new_booking_from_pro(booking, is_patient.id)
                serialized_new_booking = new_booking.serialize()
                socketio.emit('update_calendar', serialized_new_booking, room=serialized_new_booking['clinic_id'])
                return jsonify({'message': 'Booking created', 'data': new_booking.serialize()}), 200
            except Exception as e:
                return jsonify({"error": f"Error while creating new booking: {str(e)}"}), 500
        if not is_patient:
            try:
                new_patient = create_new_patient(patient)
                new_booking = create_new_booking_from_pro(booking, new_patient.serialize()['id'])
                serialized_new_booking = new_booking.serialize()
                socketio.emit('update_calendar', serialized_new_booking, room=serialized_new_booking['clinic_id'])
                return jsonify({'message': 'Booking created', 'data': new_booking.serialize()}), 200
            except Exception as e:
        ...
        return jsonify({'error': 'Bad Request - Missing patient or booking data'}), 400
    
# Delete booking from pro
...
def delete_booking(booking_id):
    ...
    if booking:
        try:         
            serialized_booking = booking.serialize()
            db.session.delete(booking)
            db.session.commit()
            return jsonify({'message': 'Booking has been deleted', 'data': f'{serialized_booking}'})
        except Exception as e:
            return jsonify({"error": f"Error while deleting booking: {str(e)}"}), 500


# Post new booking through patient calendar
@api.route("/new_booking", methods=['POST'])
def handle_new_booking():
    data = request.json
    patient = data.get('contactInfo')
    date = data.get('date')
    hub_service_id = data.get('serviceHubId')
    patient_email = patient['email']
    patient_notes = patient['patientNotes']
    is_patient = Patients.query.filter_by(email=patient_email).first()
    asociated_clinic = Clinics.query.join(Hubs).join(HubServices).filter_by(id=hub_service_id).first()
    asociated_pro = Pros.query.get(asociated_clinic.owner_id)
    serialized_pro = asociated_pro.serialize()
    current_date = get_current_date(asociated_clinic.id)
    if not is_patient:
        try:
            new_patient = create_new_patient(patient)
        except Exception as e:
            return jsonify({"error": f"Error while creating new patient: {str(e)}"}), 500
        
        try:
            booking_taken = Bookings.query.filter_by(date=datetime.strptime(date, '%Y-%m-%dT%H:%M'), hub_service_id=hub_service_id).first()
            if booking_taken:
                return jsonify({'error': 'Booking is already taken'}), 409
            else:
                pass
        except Exception as e:
            return jsonify({"error": f"Error while checking if booking exist: {str(e)}"}), 500

        try:
            new_booking = create_new_booking(date, patient_notes, hub_service_id, new_patient.id) 
            booking_confirmation_email( new_booking.serialize(), new_patient.serialize())
            return jsonify({"message": "Booking created", "data": new_booking.serialize()}), 200
        except Exception as e:
            return jsonify({"error": f"Error while creating new booking: {str(e)}"}), 500
    ...
    if is_patient:
        try:
            if patient['email'] != is_patient.email or \
            patient['name'] != is_patient.name or \
            patient['phone'] != is_patient.phone or \
            patient['privacyPolicy'] != is_patient.privacy_policy or \
            patient['marketingPolicy'] != is_patient.marketing_policy :
                update_patient(patient, is_patient)
        except Exception as e:
            return jsonify({"error": f"Error while updating patient: {str(e)}"}), 500
        ...
        try:
            if serialized_pro['options']['patientBookingLimit'] and serialized_pro['options']['patientBookingLimit'] == True:
                already_booked = Bookings.query.filter_by(patient_id=is_patient.id, hub_service_id=hub_service_id).filter(Bookings.date > current_date).first()
                if already_booked:
                    return jsonify({"error": "Patient has already a booking within this hub service. Please cancel it before booking another one.", "data": already_booked.serialize()}), 409
        except Exception as e:
            return jsonify({"error": f"Error while checking if patient has already booked with this service: {str(e)}"}), 500    

        try:
            new_booking = create_new_booking(date, patient_notes, hub_service_id, is_patient.id) 
            booking_confirmation_email( new_booking.serialize(), is_patient.serialize())
            return jsonify({"message": "Booking created", "data": new_booking.serialize()}), 200
        except Exception as e:
            return jsonify({"error": f"Error while creating new booking: {str(e)}"}), 500
        

# Bookings by patient and pro
@api.route('/patient_bookings/<int:patientid>/pros/<int:proid>', methods=['GET'])
def patient_bookings(patientid, proid):
    patient = Patients.query.get(patientid)
    bookings = Bookings.query.filter_by(patient_id=patientid).join(HubServices).join(Hubs).filter_by(pro_id=proid).all()
    owned_clinics = Clinics.query.filter_by(owner_id=proid).all()
    for clinic in owned_clinics:
        bookings.extend(Bookings.query.filter_by(patient_id=patientid).join(HubServices).join(Hubs).filter_by(clinic_id=clinic.id).filter(Hubs.pro_id != proid).all())
    if not bookings:
        return jsonify({"message": "No bookings found for this patient"}), 404
    return jsonify({"message": "Bookings downloaded", "data": {"bookings": [booking.serialize() for booking in bookings], "patient": patient.serialize()}})

# Booking confirmation
@api.route('/booking_confirmation/<int:bookingid>', methods=['PUT'])
def booking_confirmation(bookingid):
    try:
        booking = Bookings.query.get(bookingid)
        if not booking:
            return jsonify({"error": "Booking is no longer available"}), 404
        patient = Patients.query.get(booking.patient_id)
        form = False
        if patient and patient.form:
            form = {
                'allergies': bool(patient.form.allergies_id),
                'medications': bool(patient.form.medications_id),
                'diseases': bool(patient.form.diseases_id),
                'surgeries': bool(patient.form.surgeries_id)
            } 
        booking.status = 'confirmed'
        db.session.commit()
        serialized_booking = booking.serialize()
        socketio.emit('update_calendar', serialized_booking, room=serialized_booking['clinic_id'])

        return jsonify({"message": "Booking confirmed succesfully!", "data": {"patientId": booking.serialize()['patient_id'], "form": form}}), 200
    except Exception as e:
            return jsonify({"error": f"Error while confirming booking: {str(e)}"}), 500
